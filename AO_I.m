...
GaussianRandomizationNumber = Par.GaussianRandomizationNumber;
...
for num = 1:number
    ...
    for i = 1:length(Pt_dBm_range)
        ...
        Pt = 10^(Pt_dBm/10-3);
        phi = phi_all(:,:,i,num);
        [max_mse,B,Phi,R] = SDRSolver(Hhat_sd,Hhat_sr,Hhat_rd,...
...
save(['./Result/NMSE=',num2str(NMSE),'SDRTransceiverResult.mat'],...
